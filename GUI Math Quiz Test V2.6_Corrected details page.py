...
class Math():
    ...
    def Check(self):
        ...
        if count == 5:
            ...
            data.close()           
            #Adds 2D list to scorepage
            self.all_details = []
            for line in range(len(list_of_lists)):
                #Adds Name
                info = Label(self.Details_page, text = list_of_lists[line][0], font = ("comic sans ms", "11"))
                self.all_details.append(info)
                info.grid(row = line+2, column = 0)  
                #Adds Age
                info1 = Label(self.Details_page, text = list_of_lists[line][1], font = ("comic sans ms", "11"))
                self.all_details.append(info1)
                info1.grid(row = line+2, column = 1) 
                #Adds score
                info2 = Label(self.Details_page, text = list_of_lists[line][2], font = ("comic sans ms", "11"))
                self.all_details.append(info2)
                info2.grid(row = line+2, column = 2) 
            #Opens user score page
            self.End()
